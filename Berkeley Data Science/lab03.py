...
earliest_movie_title = imdb_by_year.row(0)[2]
...
first = imdb.where('Year',are.between(1900,2000))
average_20th_century_rating =  np.average(first.column('Rating'))
second = imdb.where('Year',are.between(2000,2100))
average_21st_century_rating = np.average(second.column('Rating'))
...
first = imdb.where('Year',are.between(1900,2000))
proportion_in_20th_century = first.num_rows/imdb.num_rows
second = imdb.where('Year',are.between(2000,2100))
proportion_in_21st_century = second.num_rows/imdb.num_rows
...
remainder =  imdb.column('Year')%2
imdb = imdb.with_column('Remainder',remainder)
num_even_year_movies = len(imdb.where('Remainder', 0).column('Remainder'))
...
population_above_6_billion = population.where('Population', are.above(6000000000))
year_population_crossed_6_billion = population_above_6_billion.sort('Year').column('Year')[0]
...
berkeley_markets = farmers_markets_locations.where('city', 'Berkeley')
berkeley_markets
